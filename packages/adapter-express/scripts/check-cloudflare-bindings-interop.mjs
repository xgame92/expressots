...
const servicesFactory = Symbol.for("@expressots/adapter-express/cloudflare-services-factory");
const servicesFromCjs = cjsBindings[servicesFactory]({ SETTINGS: kv });

assert.equal(servicesFromCjs.get(tokenFromEsm), kv);
assert.equal(esmBindings.kv("SETTINGS"), tokenFromEsm);
assert.equal(cjsBindings.kv("SETTINGS"), tokenFromCjs);
assert.equal(
  tokenFromEsm[Symbol.for("@expressots/adapter-express/cloudflare-binding-token")],
  true,
);
